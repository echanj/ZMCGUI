...
# 
# v2.1 this version tested and modified to be working on windows python running from cygwin terminal 
...
# tricky syntax - remove any bonds from centriod then attach centroid dummy to next atom in list   
  centroid = mol1.GetAtom(1)       
  for bond in openbabel.OBAtomBondIter(centroid): 
    print 'bond'+str(bond.GetId()) 
    mol1.DeleteBond(bond)
  mol1.AddBond(1, 2, 1)  

# now send back out as mol2 format 
  obConv.SetOutFormat("mol2")
  obConv.WriteFile(mol1, outname)
